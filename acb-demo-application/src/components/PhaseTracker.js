  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '22px 24px 20px', boxShadow: 'var(--shadow)', marginBottom: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '11px', color: 'var(--text-3)' }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}><span style={{ width: '9px', height: '9px', borderRadius: '50%', background: '#16A34A' }}></span>Completed</span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}><span style={{ width: '9px', height: '9px', borderRadius: '50%', background: '#2563EB' }}></span>In Progress</span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px' }}><span style={{ width: '9px', height: '9px', borderRadius: '50%', background: '#94A3B8' }}></span>Not Started</span>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'flex-start' }}>
        {trackerItems.map((s, i) => (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', minWidth: 0 }}>
            ...
            <button onClick={() => navigate(`/case/${caseId}/${s.id}`)} style={{ position: 'relative', zIndex: 2, width: '36px', height: '36px', borderRadius: '50%', background: s.circleBg, border: `2px solid ${s.circleBorder}`, color: s.circleColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700, boxShadow: s.ring, transition: 'box-shadow 150ms ease', cursor: 'pointer' }}>
